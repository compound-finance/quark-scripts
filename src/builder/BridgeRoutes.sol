...
library BridgeRoutes {
        ...
        returns (Bridge memory)
    {
        if (srcChainId == 1) {
            ...
            return Bridge({bridgeAddress: address(0), bridgeType: BridgeType.NONE});
            // revert BridgeNotFound(1, dstChainid, assetSymbol);
        }
    }

    function getBridgeForMainnet(uint256, /*dstChainId*/ string memory assetSymbol)
        internal
        pure
        returns (Bridge memory)
    {
        if (Strings.stringEqIgnoreCase(assetSymbol, "USDC")) {
            return Bridge({bridgeAddress: 0xBd3fa81B58Ba92a82136038B25aDec7066af3155, bridgeType: BridgeType.CCTP});
        } else {
            return Bridge({bridgeAddress: address(0), bridgeType: BridgeType.NONE});
            // revert BridgeNotFound(1, dstChainid, assetSymbol);
        }
    ...
    function getBridgeForBase(uint256, /*dstChainId*/ string memory assetSymbol)
        internal
        pure
        returns (Bridge memory)
    {
        if (Strings.stringEqIgnoreCase(assetSymbol, "USDC")) {
            return Bridge({bridgeAddress: 0x1682Ae6375C4E4A97e4B583BC394c861A46D8962, bridgeType: BridgeType.CCTP});
        } else {
            return Bridge({bridgeAddress: address(0), bridgeType: BridgeType.NONE});
            // revert BridgeNotFound(1, dstChainid, assetSymbol);
        }
