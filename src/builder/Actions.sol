...
import {CometSupplyActions, CometWithdrawActions, TransferActions} from "../DeFiScripts.sol";
...
library Actions {
        ...
        returns (IQuarkWallet.QuarkOperation memory, Action memory)
    {
        ...
        Accounts.QuarkState memory accountState = Accounts.findQuarkState(cometWithdraw.withdrawer, accounts.quarkStates);
        ...
        Action memory action = Actions.Action({
            ...
            paymentMethod: payment.isToken ? PAYMENT_METHOD_PAYCALL : PAYMENT_METHOD_OFFCHAIN,
            // Null address for OFFCHAIN payment.
            paymentToken: payment.isToken ? PaymentInfo.knownToken(payment.currency, cometWithdraw.chainId).token : address(0),
